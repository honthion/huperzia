...
class AccountDao {
// 获取首页数据
  static getIndexDao(Store store, {since = 'daily', languageType, page = 0, needDb = true}) async {
  ...
  static getRepositoryDetailDao(userName, reposName, branch, {needDb = true}) async {
    ...
    next() async {
      ...
      if (res != null && res.result && res.data.length > 0) {
        ...
        var issueResult = await AccountDao.getRepositoryIssueStatusDao(userName, reposName);
...
//      }
  ...
  static getNewsVersion(context, showTip) async {
    ...
    var res = await getGnucashIndexDao();
    if (res != null && res.result) {
